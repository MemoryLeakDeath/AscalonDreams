...
vec4 fxaa(sampler2D tex, vec2 uv) {
    ...
    // divide dir by the distance to the nearest edge plus reduction
    ...
    // get lowest and higest luma values
    ...
    // if the average is outside the luma range, use the middle average
    return ((lumaB < lumaMin) || (lumaB > lumaMax)) ? A : B;
}

void main() {
    if(USE_FXAA == 0) {
        outFragmentColor = gamma(texture(inputTexture, inTextureCoordinates));
        return;
    }
    outFragmentColor = fxaa(inputTexture, inTextureCoordinates);
    outFragmentColor = gamma(outFragmentColor);
}
