...
public class VulkanScene {
   ...
   private static VulkanScene vulkanScene;

   private VulkanScene(VulkanWindow window) {
      this.projection = new Projection(window.getFov(), window.getZNear(), window.getZFar(), window.getWidth(), window.getHeight());
      this.camera = new Camera();
   }

   public static VulkanScene createInstance(VulkanWindow window) {
      vulkanScene = new VulkanScene(window);
      return vulkanScene;
   }

   public static VulkanScene getInstance() {
      return vulkanScene;
   }
