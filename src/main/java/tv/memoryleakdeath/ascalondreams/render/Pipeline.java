...
public class Pipeline {
   ...
   public Pipeline(LogicalDevice device, PipelineCache pipelineCache, PipelineBuildInfo info) {
      ...
      try (var stack = MemoryStack.stackPush()) {
         ...
         this.id = StructureUtils.createGraphicsPipelineInfo(stack, device, info.colorFormats(), shaderStages, info.info(), layoutId,
                 pipelineCache, info.depthFormat(), info.useBlend(), info.depthClamp());
      }
   }
